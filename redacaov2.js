(function() {
    ...
    try {
        ...
        function addBookmarkletStyles() { try { const s = document.createElement("style"); s.type = "text/css"; s.innerText = styles; document.head.appendChild(s); logToMemory("Styles injected", "debug"); } catch (e) { console.error(`${SCRIPT_NAME} StyleErr:`, e); logToMemory(`Style injection error: ${e}`, "error"); } }
        ...
        function createUI() {
            logToMemory('createUI called', 'debug'); if (!document.body) { logToMemory('document.body not ready in createUI', 'error'); return; } if (document.getElementById('hck-toggle-button')) { logToMemory('UI already exists', 'debug'); return; }
            try {
            ...
            } catch (e) { logToMemory(`Error creating UI: ${e}`, 'error'); alert('[HCK] Falha ao criar a interface.'); }
        }
    ...
    function initialize() {
        ...
        try {
            ...
            logToMemory(`Initialize Error: ${initError}`, 'error');
            console.error("[HCK] Initialize Error:", initError);
            alert("[HCK] Erro na inicialização. Verifique o console.");
        }
    }
    ...
    if (document.readyState === 'loading') { logToMemory("Esperando DOM", 'debug'); document.addEventListener('DOMContentLoaded', initialize); } else { logToMemory("DOM pronto, init c/ delay.", 'debug'); setTimeout(initialize, 250); }
        ...
        console.error('[HCK] CRITICAL GLOBAL ERROR:', globalError);
        alert(`[HCK] Erro crítico no script: ${globalError.message}. Verifique o console (F12).`);
    }
})();
